(function() {
  ...
  define("kopi/app", function(require, exports, module) {
    var $, App, appInstance, array, baseURL, events, hist, klass, loc, logger, logging, overlays, settings, support, text, uri, utils, viewport, win;
    $ = require("jquery");
    ...
    */
    ...
    App = (function(_super) {
      ...
      function App(options) {
        ...
        if (appInstance) {
          throw new Error("Only one app can be initialized in current page");
        }
        appInstance = this;
        self = this;
      ...
      */
      ...
      App.prototype._match = function(url) {
        ...
        if (!this._router) {
          return logger.warn("Router is not provided");
        }
        self = this;
        path = uri.parse(url).path;
        request = this._router.match(path);
        if (!request) {
          return logger.warn("Can not find proper route for path: " + path);
        }
        route = request.route;
        _ref = self._views;
        for (guid in _ref) {
