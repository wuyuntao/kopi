    ...
    */
    ...
    Logger = (function() {
      ...
      defineMethod = function(logger, name) {
        var methodName, upper;

        if (logger[name] && !logger[name].isNoop) {
          return;
        }
        upper = name.toUpperCase();
        methodName = name === "log" && console && (console.debug != null) ? "debug" : name;
        return logger[name] = function() {
          var seconds;

          seconds = Math.round(new Date() - startTime) / 1000;
          if (console) {
            console[methodName].apply(console, ["[" + upper + "] [" + seconds + "s] [" + this._name + "]"].concat(__slice.call(arguments)));
          }
        };
      };
